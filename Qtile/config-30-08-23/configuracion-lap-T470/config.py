...
screens = [
    Screen(
        top=bar.Bar(
            [
               ...
               # widget.Chord(chords_colors={"launch": ("#ff0000", "#ffffff"),},name_transform=lambda name: name.upper(),),
                ...
                #Widget sistema
                widget.Systray(background=color_barra,fontsize=2,foreground="#F44611"),
                widget.Sep(linewidth=0,padding=1,foreground="#e9d0de",background=color_barra,**powerline2), #Separacion
                #Info CPU
		        widget.Sep(linewidth=0,padding=3,foreground="#9999FF",background=color_barra,**powerline2), #Separacion
		        widget.CPU(format="CPU:{load_percent}%",foreground="#000000",background="#9999FF",font=fuente_barra),
		        widget.TextBox(text="T:",foreground="#000000",background="#9999FF",font=fuente_barra,padding_y=3),
                widget.ThermalSensor(foreground="#000000",background="#9999FF",font=fuente_barra,threshold=50,**powerline),
		...
		#Brillo lap
		widget.TextBox(text="Brt:",foreground="#000000",background="#66CCFF",font=fuente_barra,padding_y=3),
		widget.Backlight(foreground="#000000" ,font=fuente_barra, background="#66CCFF",**powerline2,backlight_name="intel_backlight"),		
       		widget.Sep(linewidth=0,padding=1,foreground="#e9d0de",background=color_barra,**powerline2), #Separacion
	        
                #Info Memoria ram
                widget.Memory(format='RAM: {MemUsed: .0f}{mm}',foreground='#000000',background="#99FFFF",font=fuente_barra,fontsize=13,update_interval=5,**powerline ), 		
		        widget.Sep(linewidth=0,padding=1,foreground="#e9d0de",background=color_barra,**powerline), 
                
                #Hora y fecha
		        widget.Clock(format="%d-%m %I:%M %p",foreground="#000000",background="#66FFCC",font=fuente_barra,**powerline2),		
		        widget.Sep(linewidth=0,padding=1,foreground="#66CCCC",background=color_barra,**powerline2),
    
                #volumen
                #widget.TextBox(text="Vol:",foreground="#000000",background="#eed290",font=fuente_barra),
                widget.Volume(foreground="#000000",background="#99FF99",emoji=True,font=fuente_barra),
                widget.Volume(foreground="#000000",background="#99FF99",font=fuente_barra,**powerline),
                widget.Sep(linewidth=0,padding=1,foreground="#e9d0de",background=color_barra,**powerline), 
		
                #Red
                widget.Net(format='{down}↓↑',font=fuente_barra,foreground="#000000", background="FFFF99",**powerline2),
		        widget.Sep(linewidth=0,padding=1,foreground="#e9d0de",background=color_barra,**powerline2),

                
                #Bateria
                widget.Battery(format="{char} {percent:2.0%} ({hour:d}:{min:02d})",battery=0,foreground="#000000",background="#FF9966",font=fuente_barra),
                widget.Battery(format=" {percent:2.0%} ({hour:d}:{min:02d})",battery=1,foreground="#000000",background="#FF9966",font=fuente_barra,**powerline),
                widget.Sep(linewidth=0,padding=1,foreground="#e9d0de",background=color_barra,**powerline), 

                #Layout
		widget.CurrentLayout(foreground="#000000",background="#FF9999",font=fuente_barra,**powerline2),
		widget.Sep(linewidth=0,padding=1,foreground="#e9d0de",background=color_barra,**powerline2),
